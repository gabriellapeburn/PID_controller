...
# This is the timer function that runs at the desired rate from above
    ...
    #     #This is an example on how to create a message, fill in the header information and add some data, then publish
        ...
        error_output = self.PID(Kp, Ki, Kd)
        
        #publish cmd
        msg_out = WrenchStamped()
        msg_out.header = self.get_clock().now().to_msg()
        msg_out.force.x = self.desired_speed
        msg_out.torque.z = error_output
    ...
    def PID(Kp, Ki, Kd):
        ...
        self.Ki = self.get_parameter('Ki')
        p = self.Kp*error
        i = self.Ki*error*(d_time)
        d = self.Kd*error/(d_time)
        controller = p + i + d
        return controller
